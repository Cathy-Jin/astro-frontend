...
const Home = () => {
  ...
  const [isLifeThemeResultCollapsed, setIsLifeThemeResultCollapsed] = useState(false);
...
function renderResult(result, isDominantPlanetResultCollapsed, toggleDominantPlanetResultCollapse, isLifeThemeResultCollapsed, toggleLifeThemeResultCollapse) {
  // Handle errors first
  ...
  if (typeof result === 'object' && result !== null && result.hasOwnProperty('themes') && result.hasOwnProperty('planet_focalizers')) {
    return (
      <>
        {renderPlanetaryFocalizers(result.planet_focalizers, isDominantPlanetResultCollapsed, toggleDominantPlanetResultCollapse)}
        <br />
        {renderThemes(result.themes, isLifeThemeResultCollapsed, toggleLifeThemeResultCollapse)}
      </>
    );
  } else {
    return (
...
function renderThemes(themes, isLifeThemeResultCollapsed, toggleLifeThemeResultCollapse) {
  if (themes.length === 0) {
    return (
      <> 
    ...
    return (
      <> 
        <div className="report_item">
          <h2>人生主题 <button className="collapse-btn" onClick={toggleLifeThemeResultCollapse}>{isLifeThemeResultCollapsed ? '+' : '-'}</button></h2>
          <div className="report_item_summary">
            <p>在本命盘上出现三次及以上的占星规则的组合，就是人生主题。人生主题可以帮助认识自我以及指明成长的方向。计算结果按照能量出现的次数<b>从高到低</b>排列。对本命盘的进一步分析应结合太阳、月亮、上升点等其他因素综合判断。</p>
          </div>
          <div className={`content ${isLifeThemeResultCollapsed ? 'hidden' : ''}`}>
...
function renderEnergy(e) {
  return (
      <>
          <h3>主题：{e.name_cn}</h3>
          <p><b>关键词（仅供参考，需要结合星盘全局分析）</b>：{renderEnergyKeywords(e.principle_kws_cn, e.principle_nums)}</p>
          <p><b>解读（仅供参考，需要结合星盘全局分析）</b>：<br />{e.interpretation_cn}</p>
...
function renderPlanetaryFocalizers(focalizers, isDominantPlanetResultCollapsed, toggleDominantPlanetResultCollapse) {
  if (focalizers.length === 0) {
    ...
    return (
      <> 
        <div className="report_item">
          <h2>重点行星 <button className="collapse-btn" onClick={toggleDominantPlanetResultCollapse}>{isDominantPlanetResultCollapsed ? '+' : '-'}</button></h2> 
          <div className="report_item_summary">
            <p>重点行星是本命盘中能量强大的行星，对全局有较为突出的影响。对本命盘的进一步分析应结合太阳、月亮、上升点等其他因素综合判断。</p>
          </div>
          <div className={`content ${isDominantPlanetResultCollapsed ? 'hidden' : ''}`}>
            {!isDominantPlanetResultCollapsed && renderFocalizersContent(focalizers)}
          </div>
        </div>
      </>
    );
    ...
    <>
        ...
        <p>
          ...
          {domPlanets.length === 1 ? (
              ...
              <ul>
                {domPlanets.map(planet => (<li key={planet.id}>{planet.interpretation_cn}</li>))}
              </ul>
            )
          }
        </p>
...
function renderPlanetFocalizerTableRow(focalizers, domScore, name_cn) {
  ...
  return (
    <tr>
      <td>{name_cn}</td>
      <td>{focalizer.score === domScore ? <b>{focalizer.score}</b> : focalizer.score}</td>
      <td className='focalizer_pattern_row'>{focalizer.patterns_cn.join('；')}</td>
    </tr>
  );
}
